...
| Phase 3 | ✅ | Claude | 全部分项已完成,见下 |
| P3-1 CLAUDE.md 与实际对齐 | ✅ | Claude | 删除 `components/` 行误描的 `QueryBuilder + ConditionNode`(实际仅 `AppLayout.vue`),新增 `utils/safeUrl.ts` 一行;修正软删条目反映 P1-5 后的实际状态;更新 RecordsView/GovernanceView 行数(~836 / ~1241);`types/api.ts` 注释改为"已对齐";已知陷阱补 `.dockerignore` 唯一权威副本提示。 |
| P3-2 /health + /ready DB ping | ✅ | Claude | 新增 `backend/internal/handlers/health.go`:`Health` 是 liveness(纯返回 200,不查依赖),`Ready` 是 readiness(用 `pkgdb.DB().DB().PingContext(2s 超时)` 校验 DB,挂时返 503 并带 `reason` 字段说明)。版本号经 `handlers.SetVersion(Version)` 注入,避免 handler 反向依赖 `main`。`cmd/server/main.go` 把原内联匿名 handler 拆成 `r.GET("/health", handlers.Health)` + `r.GET("/ready", handlers.Ready)`。`backend/Dockerfile` HEALTHCHECK 和 `docker-compose.yml` 的 app.healthcheck 全部切到 `/ready`,start_period 同步抬到 10s 给 DB 留迁移时间。新增 `handlers/health_test.go`:3 个用例(Health 200 + Ready 200 + 关 DB 后 Ready 503),`go test ./internal/handlers/...`(`-skip TestHandleMCPRealHTTP`)全绿,`go vet` 干净。 |
| P3-3 LLM Governor 重试 + 熔断 | ✅ | Claude | `backend/internal/services/llm_governor_client.go` 加 `retryConfig`(默认 MaxAttempts=3、BaseDelay=200ms 指数退避)+ `circuitBreaker`(5 次连续失败开 30s 熔断, 命中后直接返 `ErrCircuitOpen` 不再发起 HTTP)。新增 `transientHTTPError` 区分 5xx/429(可重试)与 4xx(立即返); `isRetryable` 在 ctx 取消/超时时不计入熔断, 保证调用方的边界决策不被误判为上游故障。`ctx.Done` 在重试 sleep 中也尊重(避免 30s sleep 阻塞优雅关闭)。新增 `llm_governor_client_test.go`:7 个用例覆盖 5xx 最终成功 / 4xx 不重试 / 重试耗尽 / 阈值开熔断 / 冷却后自动闭合 / 成功清零计数 / ctx 超时不入计数, `go test ./internal/services/...` 全绿。无新依赖(用标准库 sync.Mutex + 标准 time.After), 实现是最小化连续失败熔断器, 注释说明了为何不上 half-open 探测/失败率窗口。 |
| P3-4 大列表分页 / 虚拟滚动 | ✅ | Claude | **后端**: Governance/Organization/Plugin 三个 list 接口统一加上 page/page_size 参数(默认 page=1, pageSize=20, 上限 200, pageSize=-1 为跳过分页的逃生口),返回 `{items,total,page,page_size}`。handler 通过 `strconv.Atoi(c.Query("page"))` 解析;service 先 `Count` 再 `LIMIT/OFFSET`,保持原 `ORDER BY created_at DESC`。**前端**: `api.ts` 的 `organizationAPI.list` 和 `pluginAPI.list` 改接受可选 params;`types/api.ts` 新增 `PluginListResponse`,并为 `OrganizationListResponse`/`GovernanceTaskListResponse` 补 `page`/`page_size`。三个 view 新增分页状态(currentPage, pageSize, total)和 `<el-pagination>`;filter 变化或增删改后自动回到第 1 页。`RecordsView` 已具备服务端分页(Limit/Offset),未改用 `el-table-v2`(现有 pageSize≤100 的 `el-table` 尚未构成渲染瓶颈,重写为 el-table-v2 的 ROI 不匹配)。测试同步修复: `organization_service_test.go`×2 / `governance_test.go`×2 调用点改用 `.Items` 和 `.Total`;`PluginsView.spec.ts` 的 list mock 由 `[...]` 改为 `{items:[...],total:1,page:1,page_size:20}`。`go test ./internal/services ./internal/handlers`(`-skip TestHandleMCPRealHTTP`)全绿;`pnpm type-check` + `pnpm test:unit --run` 62/62 通过。 |
| P3-5 axios 超时分级 | ✅ | Claude | `services/api.ts` 抽出 `LONG_TIMEOUT_MS = 60000` 常量,全局 `API_CONFIG.timeout = 10000` 保留。**新增** `fileAPI.upload({ recordId, fieldId?, file }, onUploadProgress?)`(60s,FormData + multipart 头由方法内部组装);**改** `exportAPI.downloadRecords` 增加 `timeout: 60000`;**新增** `governanceAPI.generateAIRecommendations({task_id, recommendation_type, resource_type?, resource_id?, context?})` 60s,直连后端 `POST /governance/ai/recommendations`。`RecordsView.vue` 删除原 inline `apiInstance.post('/files/upload', formData, ...)`,改走 `fileAPI.upload`,保留 progress 回调。`RecordsView.spec.ts` 同步:`fileAPI` mock 加 `upload: vi.fn()`,upload 用例换为对 `fileAPI.upload` 断言。`pnpm type-check` 通过,`pnpm test:unit --run` 62/62 通过,`pnpm exec eslint --max-warnings=0` 干净。 |
| P3-6 可观察性 + AGPL 合规 | ✅ | Claude | **可观察性**:`cmd/server/main.go` 新增 `r.GET("/metrics", gin.WrapH(promhttp.Handler()))`,默认暴露 Go runtime + process collector 指标(新增依赖 `github.com/prometheus/client_golang`);**日志**:`pkg/log/zap.go` 重写为单 JSON-stdout core,删除 lumberjack 与文件输出(归 docker / k8s sidecar 管轮转),`config.LoggerConfig` 同步瘦身为只剩 `Level`,删 `LOG_OUTPUT/LOG_ERROR/LOG_MAX_*` 环境变量与 `docker-compose.yml` 的 `backend_logs` 卷,`migrate.go` 凭据保护注释里的"lumberjack 归档"措辞更新;**AGPL**:`frontend/src/components/AppLayout.vue` 侧边栏底部新增 "Source Code (AGPL-3.0)" 链接到 `https://github.com/jiangfire/cornerstone`(沿用 `utils/safeUrl.ts` 做协议白名单),侧栏折叠时只显示图标。`go build ./...` 通过,`go test ./internal/handlers ./internal/services ./internal/config ./internal/db`(`-skip TestHandleMCPRealHTTP`)全绿,`pnpm type-check` + `pnpm test:unit --run` 62/62 通过。 |
