...
- 记录的“查看 / 导出”会按字段级 `Read` 权限过滤不可见字段。
- 记录的“创建 / 更新 / 批量创建”会校验字段级 `Write` 权限。
- 文件元数据、下载、附件列表按记录读权限鉴权；上传和删除按记录写权限鉴权。
...
- 数据库级权限是基础入口，表级权限继承数据库角色，不单独维护一套表成员关系。
- 字段级权限是覆盖规则，不配置时走默认权限：
  - Owner/Admin: 默认可读可写可删
  - Editor: 默认可读可写，不可删
  - Viewer: 默认只读
- 字段权限不仅影响字段列表接口，也影响记录创建、更新、列表、详情和导出链路。
- 系统设置不属于数据库权限体系，而是系统管理员专属能力。
...
也就是说，没有字段读权限的字段不会出现在列表、详情和导出里；没有字段写权限的字段不能通过记录接口写入。

### Q: 系统设置属于哪一层权限？

A: 不属于数据库/表/字段三层权限。系统设置由系统管理员单独管理，只有 `is_system_admin=true` 的用户可以访问 `/api/settings`。
...
- ✅ 修正文档中关于数据库分享、成员角色管理、字段权限配置主体的旧描述
- ✅ 补充字段权限已落到记录读写/导出链路
- ✅ 补充附件接口按记录读写权限鉴权
- ✅ 补充系统管理员与系统设置权限边界
